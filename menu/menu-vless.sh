...
BURIQ () {
    ...
    data=( `cat /root/tmp | grep -E "^#vl# " | awk '{print $2}'` )
    for user in "${data[@]}"
    do
    exp=( `grep -E "^#vl# $user" "/root/tmp" | awk '{print $3}'` )
    d1=(`date -d "$exp" +%s`)
    d2=(`date -d "$biji" +%s`)
    exp2=$(( (d1 - d2) / 86400 ))
    if [[ "$exp2" -le "0" ]]; then
